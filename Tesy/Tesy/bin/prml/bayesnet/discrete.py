...
class DiscreteVariable(RandomVariable):
    ...
    def __repr__(self):
        ...
        if self.is_observed:
            string += "observed={proba})".format(proba = self.proba)
        else:
            string += "proba={selfproba})".format(selfproba = self.proba)
        return string
    ...
    def observe(self, data, proprange=-1):
        """
...
class DiscreteProbability(ProbabilityFunction):
    ...
    def __init__(self, table, *condition, **kwargs):
        """
        initializepp discrete probability function
...
def discrete(table, *condition, **kwargs):
    """
    discrete probability function
