...
class Bernoulli(RandomVariable):
    ...
    def mu(self, mu):
        if isinstance(mu, (int, float, np.number)):
            if mu > 1 or mu < 0:
                raise ValueError("mu must be in [0, 1], not {mu}".format(mu=mu))
            ...
            if mu is not None:
                raise TypeError("{mu} is not supported for mu".format(mu=type(mu)))
    ...
    def _map(self, X):
        ...
        assert X.size == n_zeros.sum() + n_ones.sum(), (
            "{size} is not equal to {n_zeros} plus {n_ones}".format(size=X.size, n_zeros=n_zeros, n_ones=n_ones)
        )
