...
class MatMul(Function):
    ...
    def forward(self, x, y):
        ...
        if isinstance(self.x, Constant) and isinstance(self.y, Constant):
            return Constant(np.matmul(x.value , y.value))
        return Tensor(np.matmul(x.value, y.value), function=self)

    def backward(self, delta):
        dx = np.matmul(delta, self.y.value.T)
        dy = np.matmul(self.x.value.T, delta)
        self.x.backward(dx)
        self.y.backward(dy)
