...
class VariationalLogisticRegression(LogisticRegression):
    ...
    def fit(self, X, t, iter_max=1000):
        ...
        for _ in range(iter_max):
            ...
            self.w_var = np.linalg.inv(I / self.alpha + 2 * (np.matmul(lambda_ * X.T) , X))
            self.w_mean = np.matmul(self.w_var , np.sum(X.T * (t - 0.5), axis=1))
            xi = np.sqrt(np.sum(np.matmul(X , (self.w_var + self.w_mean * self.w_mean[:, None]) * X, axis=-1)))
