...
class EmpiricalBayesRegression(BayesianRegression):
    ...
    def fit(self, X, t, max_iter=100):
        ...
        for _ in range(max_iter):
            ...
            w_precision = self.alpha * eye + self.beta * np.matmul(X.T ,X)
            w_mean = self.beta * np.linalg.solve(w_precision, np.matmul(X.T , t))

            gamma = np.sum(eigenvalues / (self.alpha + eigenvalues))
            self.alpha = float(gamma / np.sum(w_mean ** 2).clip(min=1e-10))
            self.beta = float(
                (N - gamma) / np.sum(np.square(t - np.matmul(X , w_mean)))
            )
    ...
    def _log_likelihood(self, X, t, w):
        return -0.5 * self.beta * np.square(t -np.matmul( X , w)).sum()
