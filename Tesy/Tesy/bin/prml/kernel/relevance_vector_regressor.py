...
class RelevanceVectorRegressor(object):
    ...
    def fit(self, X, t, iter_max=1000):
        ...
        for _ in range(iter_max):
            ...
            precision = np.diag(self.alpha) + self.beta * np.matmul(Phi.T , Phi)
            covariance = np.linalg.inv(precision)
            mean = self.beta * np.matmul(covariance , np.matmul(Phi.T , t))
        ...
        self.mean = self.beta * np.matmul(self.covariance ,np.matmul(Phi.T , self.t))
    ...
    def predict(self, X, with_error=True):
        ...
        mean = np.matmul(phi , self.mean)
        if with_error:
            var = 1 / self.beta + np.sum(np.matmul(phi ,self.covariance) * phi, axis=1)
