...
class RelevanceVectorClassifier(object):
    ...
    def _map_estimate(self, X, t, w, n_iter=10):
        for _ in range(n_iter):
            ...
            g = np.matmul(X.T , (y - t)) + self.alpha * w
            H = np.matmul((X.T * y * (1 - y)) , X) + np.diag(self.alpha)
    ...
    def predict(self, X):
        ...
        label = (np.matmul(phi , self.mean) > 0).astype(np.int)
        return label
    ...
    def predict_proba(self, X):
        ...
        assert X.ndim == 2
        phi = self.kernel(X, self.X)
        mu_a = np.matmul(phi , self.mean)
        var_a = np.sum(np.matmul(phi , self.covariance) * phi, axis=1)
