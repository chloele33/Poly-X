...
class Kalman(StateSpaceModel):
    ...
    def predict(self):
        ...
        mu = np.matmul(self.system , mu_prev)
        cov = np.matmul(self.system , np.matmul(cov_prev , self.system.T)) + self.cov_system
    ...
    def filter(self, observed):
        ...
        innovation = observed - np.matmul(self.measure , mu)
        cov_innovation = self.cov_measure + np.matmul(self.measure , np.matmul(cov , self.measure.T))
        kalman_gain = np.linalg.solve(cov_innovation, np.matmul(self.measure ,cov)).T
        mu += np.matmul(kalman_gain , innovation)
        cov -= np.matmul(kalman_gain ,np.matmul( self.measure , cov))
        return mu, cov
    ...
    def smooth(self):
        ...
        gain = np.linalg.solve(cov_pred_next,np.matmul( self.system , cov)).T
        mean += np.matmul(gain , (mean_smoothed_next -np.matmul( self.system , mean)))
        cov += np.matmul(gain ,np.matmul((cov_smoothed_next - cov_pred_next) , gain.T))
        self.smoothing_gain.insert(0, gain)

    def smoothing(self, observed_sequence=None):
        if(observed_sequence):
            np.ndarray(observed_sequence)
        """
...
def kalman_filter(kalman, observed_sequence):
    np.ndarray(observed_sequence)
    Kalman(kalman)
    """
...
def kalman_smoother(kalman, observed_sequence=None):
    if(observed_sequence):
        np.ndarray(observed_sequence)
    Kalman(kalman)
    """
