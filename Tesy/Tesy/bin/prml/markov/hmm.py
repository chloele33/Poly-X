...
class HiddenMarkovModel(object):
    ...
    def expect(self, seq):
        ...
        for like in likelihood[1:]:
            f = np.matmul(forward[-1] , self.transition_proba) * like
        ...
        for like, c in zip(likelihood[-1:0:-1], constant[-1:0:-1]):
            backward.insert(0, np.matmul(self.transition_proba , (like * backward[0])) / c)
    ...
    def filtering(self, seq):
        ...
        for like in likelihood[1:]:
            p = np.matmul(posterior[-1] , self.transition_proba )* like
