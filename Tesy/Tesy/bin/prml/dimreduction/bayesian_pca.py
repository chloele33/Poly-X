...
class BayesianPCA(PCA):
    ...
    def _maximization(self, X, Ez, Ezz):
        self.W = np.matmul(X.T , np.matmul(Ez , np.linalg.inv(np.sum(Ezz, axis=0) + self.var * np.diag(self.alpha))))
        self.var = np.mean(
            np.mean(X ** 2, axis=-1)
            - 2 * np.mean(np.matmul(Ez , self.W.T * X, axis=-1))
            + np.trace((np.matmul(Ezz ,np.matmul(self.W.T , self.W))).T) / len(self.mean))
