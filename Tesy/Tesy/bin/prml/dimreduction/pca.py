...
class PCA(object):
    ...
    def eigen(self, X, *arg):
        ...
        if sample_size >= n_features:
            ...
            cov = np.cov(X)
            values, vectors = np.linalg.eigh(cov)
            vectors = (np.matmul(X.T , vectors)) / np.sqrt(sample_size * values)
        ...
        self.__M = np.matmul(self.W.T , self.W) + self.var * self.I
        self.C = np.matmul(self.W , self.W.T) + self.var * np.eye(n_features)
        if index == 0:
            self.Cinv = np.linalg.inv(self.C)
        else:
            self.Cinv = np.eye(n_features) / np.sqrt(self.var) - np.matmul(self.W , np.matmul(np.linalg.inv(self.__M) , self.W.T)) / self.var
    ...
    def em(self, X, iter_max):
        ...
        for i in range(iter_max):
            ...
            if np.allclose(W, self.W):
                break
        self.C = np.matmul(self.W , self.W.T) + self.var * np.eye(np.size(X, 1))
        self.Cinv = np.linalg.inv(self.C)

    def _expectation(self, X):
        self.__M = np.matmul(self.W.T , self.W) + self.var * self.I
        Minv = np.linalg.inv(self.__M)
        Ez = np.matmul(X ,p.matmul(self.W , Minv))
        Ezz = self.var * Minv + Ez[:, :, None] * Ez[:, None, :]
        return Ez, Ezz

    def _maximization(self, X, Ez, Ezz):
        self.W =np.matmul( X.T, np.matmul(Ez , np.linalg.inv(np.sum(Ezz, axis=0))))
        self.var = np.mean(
            np.mean(X ** 2, axis=1)
            - 2 * np.mean(np.matmul(Ez , self.W.T) * X, axis=1)
            + np.trace((np.matmul(Ezz ,np.matmul( self.W.T , self.W))).T) / np.size(X, 1))
    ...
    def transform(self, X):
        ...
        """
        return np.linalg.solve(self.__M, (np.matmul((X - self.mean) , self.W)).T).T
    ...
    def proba(self, X):
        ...
        d = X - self.mean
        return (
            np.exp(-0.5 * np.sum(np.matmul(d , self.Cinv) * d, axis=-1))
